...
{
  public class PauseSoundByIdSystem : IExecuteSystem
  {
    ...
    public PauseSoundByIdSystem(GameContext game)
    {
      _game = game;
      _changeSoundRequests = game.GetGroup(GameMatcher
        .AllOf(GameMatcher.ChangeSoundRequest, GameMatcher.PauseSoundRequest, GameMatcher.TargetId));
    }
    ...
    public void Execute()
    {
      foreach (GameEntity request in _changeSoundRequests.GetEntities(_buffer))
      {
        ...
        {
          sound.isPauseSoundRequest = true;
